...
public:
    ...
    vr::EVRInitError Activate(std::uint32_t id) override {
        ...
        stop_=false;tracker_.start([this](const HidTracker::Snapshot&s){on_pose(s);});pipe_thread_=std::thread(&HmdDevice::pipe_loop,this);log("HMD activated");return vr::VRInitError_None;
    }
...
private:
    ...
    vr::DriverPose_t make_pose(const HidTracker::Snapshot&s){vr::DriverPose_t p{};p.qWorldFromDriverRotation.w=1;p.qDriverFromHeadRotation.w=1;p.deviceIsConnected=s.connected;p.poseIsValid=s.connected&&s.calibrated;p.result=p.poseIsValid?vr::TrackingResult_Running_OK:vr::TrackingResult_Uninitialized;Quat q=conjugate(s.orientation);{std::lock_guard lock(center_mutex_);if(!centered_&&s.calibrated){center_angles_=euler_degrees(q);centered_=true;}if(centered_)q=recentered_euler(q,center_angles_);}p.qRotation={q.w,q.x,q.y,q.z};p.vecAngularVelocity[0]=s.angular_velocity.x;p.vecAngularVelocity[1]=s.angular_velocity.y;p.vecAngularVelocity[2]=s.angular_velocity.z;return p;}
